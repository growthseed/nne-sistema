...
export default function Login() {
  ...
  // Login com Google — provider Google deve estar habilitado no Supabase Auth
  // (Dashboard → Authentication → Providers). Se o membro/missionário ainda não
  // tem usuário no sistema, o auto-create pelo trigger de profiles cuida da
  // criação inicial; a primeira vez exige aprovação por um admin (papel = pendente).
  async function handleGoogleLogin() {
    ...
    if (error) {
      setError(`Falha ao iniciar login com Google: ${error.message}`)
      setLoading(false)
    }
  }
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="card space-y-5">
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-200 font-medium rounded-lg px-4 py-2.5 transition-colors"
            >
