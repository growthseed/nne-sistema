    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 p-6 text-white shadow-xl">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-green-100 text-sm">Painel Administrativo — Acompanhamento geral</p>
          </div>
        </div>
      </div>
      ...
      {alunosInativos.length > 0 && (
        <div className="card p-5 border-amber-200">
          ...
          <p className="text-xs text-gray-500 mb-3">Alunos matriculados que ainda não concluíram nenhuma lição. Considere uma visita ou contato.</p>
          <div className="space-y-1 max-h-48 overflow-y-auto">
            ...
            {alunosInativos.length > 20 && (
              <p className="text-xs text-gray-400 text-center pt-2">... e mais {alunosInativos.length - 20} alunos</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="card p-5">
          ...
          <div className="space-y-2">
            {npsData.map(n => {
              ...
              return (
                <div key={n.classe_id} className="flex items-center gap-3 py-2">
                  <span className="text-xs text-gray-700 w-40 truncate shrink-0">{turma?.nome || '—'}</span>
                  <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${color}`} style={{ width: `${pct}%` }} />
                  </div>
                  <span className={`text-sm font-bold w-10 text-right ${n.media >= 8 ? 'text-green-600' : n.media >= 6 ? 'text-amber-600' : 'text-red-600'}`}>{n.media}</span>
                  <span className="text-[10px] text-gray-400 w-16 text-right">{n.total} aval.</span>
                </div>
              )
            })}
          </div>
