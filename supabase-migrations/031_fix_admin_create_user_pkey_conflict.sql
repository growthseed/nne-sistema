-- ============================================================
-- Migration 031: Fix admin_create_user PK collision
--
-- ROOT CAUSE: trigger on_auth_user_created (migration 018) fires
-- on INSERT into auth.users and inserts into public.usuarios
-- BEFORE the RPC's own INSERT runs → duplicate PK error.
--
-- FIX: Use ON CONFLICT (id) DO UPDATE so the RPC always wins
-- with the correct papel, associacao_id, uniao_id, igreja_id.
-- ============================================================
...
BEGIN
  ...
  --    ON CONFLICT para não colidir com o trigger on_auth_user_created
  --    que pode inserir a linha antes deste comando executar.
  --    O UPDATE garante que papel, associação, união e igreja fiquem corretos
  --    (o trigger cria com papel='admin' e sem vínculos — incorreto).
